...
async function queryIvynet(lat, lng, selectedBts, bts_list_latlng) {
    ...
    document.getElementById('innermenu').innerHTML = `<p style="margin-top: 0; padding: 0;font-size: 14;">@ LAT ${Math.round(lat * 1000000) / 1000000} / LON ${Math.round(lng * 1000000) / 1000000} <br> 
        &rArr; Copertura: ${JSON.stringify(click_tecno['tecno']).replace(/"/g, "")}<br>
        &rArr; Heatmap: ${heatmap_value['height']}</p>`;
    generateChart(xValuesChart, yValuesChart);
